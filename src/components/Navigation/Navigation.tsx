...
const Navigation = ({ setAnimateTab }: PropTypes) => {
  ...
  return (
    <Box
      role="navigation"
      position={breakpoint === 'base' && 'sm' ? 'fixed' : 'relative'}
      bottom={0}
      left={0}
    >
      <Center pt="5" pb={breakpoint === 'base' && 'sm' ? '.5rem' : '0'}>
        ...
        >
          <ButtonGroup gap={breakpoint === 'base' && 'sm' ? 1 : 4}>
            {navData &&
              navData.navItems.map((item: NavigationItem, index) => (
                <Link
                  to={item.to}
                  key={item.icon}
                  onClick={() => handleNavAnimation(item)}
                >
                  <motion.div
                  ...
                  >
                    <Tooltip hasArrow label={item.tooltip} bg="grey.50">
                      <Button
                        background={
                          checkLocation(item) ? '#FFD500' : 'gray.500'
                        }
                        py="2rem"
                        _hover={
                          checkLocation(item) ? {} : { transform: 'scale(0.8)' }
                        }
                      >
                        <Image
                          src={item.icon}
